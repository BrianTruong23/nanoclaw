...
describe('control-plane runner', () => {
    ...
    it('posts a failure message and does not crash when execution fails', async () => {
        ...
        expect(client.updateTask).toHaveBeenNthCalledWith(2, 'task-backlog', {
            status: 'blocked',
            message: 'NanoClaw failed while processing task-backlog: container failed',
        });
        expect(notifyLocalMessage).toHaveBeenCalledWith('NanoClaw failed while processing task-backlog: container failed');
    });
});
//# sourceMappingURL=control-plane-runner.test.js.map