...
import { getAllChats, getAllRegisteredGroups, getAllTasks, getMessagesSince, getRecentTaskRunLogs, getRouterState, initDatabase, } from './db.js';
...
const CURATED_PLUGINS_DIR = path.join(CODEX_HOME, 'plugins', 'cache', 'openai-curated');
const REPO_SKILLS_DIR = path.join(process.cwd(), '.claude', 'skills');
let managedAgent = null;
let managedAgentStartedAt = null;
let managedAgentCommand = [];
let managedAgentStatusMessage = null;
let managedAgentLastExit = null;
let managedAgentOutput = [];
...
function readRecentEvents(limit) {
    try {
        if (!fs.existsSync(DASHBOARD_EVENTS_FILE))
            return [];
        const lines = fs
...
function appendManagedOutput(stream, chunk) {
...
function getAgentCommand() {
    ...
    const distIsFresh = fs.existsSync(distEntry) &&
    ...
    if ((!distIsFresh || process.env.NANOCLAW_PREFER_TSX === '1') &&
...
function loadRuntimeStatus() {
    return readJsonFile(DASHBOARD_RUNTIME_FILE);
}
function getStartupBlocker() {
...
function isHeartbeatFresh(runtime) {
    if (!runtime)
        return false;
...
function collectSkillSummaries() {
    const skills = [];
...
function collectAppSummaries(skills) {
    const apps = [];
...
function buildSnapshot() {
    ...
    try {
        ...
        lastAgentTimestamp = raw ? JSON.parse(raw) : {};
    }
    catch {
        lastAgentTimestamp = {};
    }
    ...
    const runtimeStatus = runtime && fresh ? runtime.status : runtime ? 'stale' : 'offline';
    const recentEvents = readRecentEvents(MAX_LOG_EVENTS).filter((entry) => runtime?.pid ? entry.pid === runtime.pid : true);
    const recentMessages = groups
        .flatMap((group) => getMessagesSince(group.jid, '', ASSISTANT_NAME, 8).map((message) => ({
    ...
    return {
        ...
        summary: {
            ...
            connectedChannels: runtime?.channels.filter((channel) => channel.connected).length ?? 0,
