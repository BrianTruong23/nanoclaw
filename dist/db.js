...
export function hasCrossBotResponse(chatJid, sinceTimestamp) {
    const row = db
        .prepare(`SELECT 1 FROM messages WHERE chat_jid = ? AND id LIKE 'xbot_%' AND timestamp > ? LIMIT 1`)
...
export function getRecentTaskRunLogs(limit = 20) {
    return db
        .prepare(`
